...
class main(QtGui.QDialog, Ui_Form):#, Player
    ...
    def on_pushButton_arm_clicked(self):
        self.textBrowser.append('arm')
        ctrl=0x4  #arm
        send_CONTROL(ctrl)

    @QtCore.pyqtSlot()
    def on_pushButton_disarm_clicked(self):
        self.textBrowser.append('disarm')
        arm=0x8  #disarm
        send_CONTROL(ctrl)

    @QtCore.pyqtSlot()
    def on_pushButton_takeoff_clicked(self):
        self.textBrowser.append('takeoff')
        ctrl=0x2  #takeoff
        send_CONTROL(ctrl)

    @QtCore.pyqtSlot()
    def on_pushButton_connect_clicked(self):
        self.textBrowser.append('ssh')
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect('127.0.0.1', 22, username='tyh', password='tyhNsFz923')
        cmd='ls'
...
##                #set_CHANNEL(throttle,roll,pitch,yaw,mode)           
##                pygame.event.get()
...
def send_CONTROL(ctrl):
    try:
        by=struct.pack("BBBBc",0xff,0xaa,0x1,0x1,ctrl)
        send_sock.sendall(by)
    except Exception as err:    pass

def send_CHANNEL(throttle,roll,pitch,yaw,mode):
    try:
        by=struct.pack("BBBBBBBBB",0xff,0xaa,0x2,0x9,throttle,roll,pitch,yaw,mode)
        send_sock.sendall(by)
    except Exception as err:    pass
    ...
    def receive_server():
        ...
        while True:
            ...
            try:
                ...
                if ((parity_bit[0]==255)and(parity_bit[1]==170)and(parity_bit[2]==0xbb)and(parity_bit[3]==0xcc)): #FFAA
                    ui.textBrowser.append('receive_server: get head!')
                    recv_bytes=receive_sock.recv(1)
                    length=struct.unpack("B", recv_bytes)
                    ui.textBrowser.append('receive_server: get len!')#length=60*1
                    #print length[0]
                    ...
                    ui.label_channel1.setText(str(chan1*10))

                    ui.label_channel2.setText(str(chan2*10))

                    ui.label_channel3.setText(str(chan3*10))

                    ui.label_channel4.setText(str(chan4*10))
...
##                    ui.label_channel8.setText(str(chan8))
                    ...
                    ui.label_rollspeed.setText(str(rollspeed)+'cm/s')

                    ui.label_pitchspeed.setText(str(pitchspeed)+'cm/s')

                    ui.label_yawspeed.setText(str(yawspeed)+'cm/s')

                    ui.label_altitude.setText(str(hud_alt)+'cm/s')

                    ui.label_climb_speed.setText(str(hud_climb)+'cm/s')

                    ui.label_ground_speed.setText(str(hud_groundspeed)+'cm/s')
...
##
##            try:
####                send_bytes=struct.pack("<4s4sIIII4s4s",
####                                       "ffaa",level,arm,takeoff,throttle,roll,pitch,yaw,mode,"6611")
####                send_sock.sendall(send_bytes)
##                    
##            except Exception as err:
