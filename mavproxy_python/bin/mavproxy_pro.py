...
# new defined member,usage:write status infomation#
        self.data_fd = open('/home/tyh/beaglebone/Program_update/data_fifo','wb')
#######################################################
...
# we add new data structure which should send to beaglebone black#
        if self.armed == True:
            ...
            self.arm = 0
        data_packed = struct.pack('iiiiiiiiiiiiiiiiiiiiiiiidddddd', \
            self.arm, \
...
status_fd = open('/home/tyh/beaglebone/Program_update/status.txt','w')
def cmd_status(args):
     '''show status'''
     if len(args) == 0:
