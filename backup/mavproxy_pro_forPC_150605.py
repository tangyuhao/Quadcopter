...
class MPStatus(object):
    ...
    def __init__(self):
        ...
        self.last_seq = 0
        self.armed = False
#new dinede:usage: whether the plane is armed#
        self.arm = 0
# new defined member,usage:write status infomation#
        self.data_fd = open('/home/tyh/QUADCOPTER_GIT/Quadcopter/data_fifo','wb')
#######################################################
...
# we add new data structure which should send to beaglebone black#
        if self.armed == True:
            self.arm = 1
        else:
            self.arm = 0
        data_packed = struct.pack('iiiiiiiiiiiiiiiiiiiiiiiifffffffff', \
            self.arm, \
            self.msgs['RAW_IMU'].xacc,\
            self.msgs['RAW_IMU'].yacc,\
            self.msgs['RAW_IMU'].zacc,\
            self.msgs['SERVO_OUTPUT_RAW'].servo1_raw,\
            self.msgs['SERVO_OUTPUT_RAW'].servo2_raw,\
            self.msgs['SERVO_OUTPUT_RAW'].servo3_raw,\
            self.msgs['SERVO_OUTPUT_RAW'].servo4_raw,\
            self.msgs['VFR_HUD'].heading,\
            self.msgs['GLOBAL_POSITION_INT'].lat, \
            self.msgs['GLOBAL_POSITION_INT'].lon, \
            self.msgs['GPS_RAW_INT'].eph, \
            self.msgs['GPS_RAW_INT'].satellites_visible, \
            self.msgs['RC_CHANNELS_RAW'].chan1_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan2_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan3_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan4_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan5_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan6_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan7_raw, \
            self.msgs['RC_CHANNELS_RAW'].chan8_raw, \
            self.msgs['SYS_STATUS'].voltage_battery, \
            self.msgs['SYS_STATUS'].current_battery, \
            self.msgs['SYS_STATUS'].battery_remaining, \
            self.msgs['ATTITUDE'].rollspeed,\
            self.msgs['ATTITUDE'].pitchspeed,\
            self.msgs['ATTITUDE'].yawspeed,\
            self.msgs['VFR_HUD'].alt, \
            self.msgs['VFR_HUD'].climb,\
            self.msgs['VFR_HUD'].groundspeed,\
            self.msgs['ATTITUDE'].roll,\
            self.msgs['ATTITUDE'].pitch,\
            self.msgs['ATTITUDE'].yaw)
###################################################################
        #write to fifo
#        print "data_packed" + str(len(data_packed))
        self.data_fd.write(data_packed)
        self.data_fd.flush()
#        self.data_fd.close()
...
status_fd = open('/home/tyh/QUADCOPTER_GIT/Quadcopter/status.txt','w')
def cmd_status(args):
     '''show status'''
     if len(args) == 0:
         mpstate.status.show(status_fd, pattern=None)
     else:
         for pattern in args:
             mpstate.status.show(status_fd, pattern=pattern)
